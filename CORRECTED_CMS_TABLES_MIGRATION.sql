# Corrected CMS Tables Migration

**File:** `supabase/migrations/create_cms_tables.sql` (FIXED VERSION)

Save this as the corrected migration and replace the broken one.
...
-- FIXED: Corrected RLS policies to use is_admin() function with proper auth_user_id column reference
...
--
-- NOTE: Uses is_admin() function which queries:
--   SELECT role FROM users WHERE auth_user_id = auth.uid()
-- This correctly maps Supabase auth.uid() to the users.auth_user_id column
...
-- ============================================================================
CREATE POLICY IF NOT EXISTS "site_settings_public_read" 
  ON site_settings FOR SELECT 
  USING (true);

CREATE POLICY IF NOT EXISTS "site_settings_admin_insert" 
  ON site_settings FOR INSERT
  WITH CHECK (is_admin());

CREATE POLICY IF NOT EXISTS "site_settings_admin_update" 
  ON site_settings FOR UPDATE
  USING (is_admin())
  WITH CHECK (is_admin());

CREATE POLICY IF NOT EXISTS "site_settings_admin_delete" 
...
-- ============================================================================
CREATE POLICY IF NOT EXISTS "cms_pages_published_read" 
  ON cms_pages FOR SELECT
  USING (is_published = true);

CREATE POLICY IF NOT EXISTS "cms_pages_admin_insert" 
  ON cms_pages FOR INSERT
  WITH CHECK (is_admin());

CREATE POLICY IF NOT EXISTS "cms_pages_admin_update" 
  ON cms_pages FOR UPDATE
  USING (is_admin())
  WITH CHECK (is_admin());

CREATE POLICY IF NOT EXISTS "cms_pages_admin_delete" 
...
-- ============================================================================
CREATE POLICY IF NOT EXISTS "cms_sections_published_read" 
...
CREATE POLICY IF NOT EXISTS "cms_sections_admin_insert" 
  ON cms_sections FOR INSERT
  WITH CHECK (is_admin());

CREATE POLICY IF NOT EXISTS "cms_sections_admin_update" 
  ON cms_sections FOR UPDATE
  USING (is_admin())
  WITH CHECK (is_admin());

CREATE POLICY IF NOT EXISTS "cms_sections_admin_delete" 
...
### 1. Corrected RLS Policies
- **Before:** `(SELECT role FROM users WHERE id = auth.uid())`
- **After:** `is_admin()` function call with proper `auth_user_id` reference

### 2. Function Dependency
- Assumes `is_admin()` function exists in your database
- Location: `supabase/migrations/20251116061131_fix_users_table_infinite_recursion_v2.sql`

### 3. Added IF NOT EXISTS to Policies
- Prevents errors if policies are re-created
- Makes migration idempotent

### 4. Enhanced Comments
- Added explanation of how RLS works
- Documented the auth.uid() to auth_user_id mapping

---

## Deployment Steps

1. **Backup current migration file**
   ```bash
   cp supabase/migrations/create_cms_tables.sql supabase/migrations/create_cms_tables.sql.backup
   ```

2. **Replace with corrected version**
   - Copy the SQL above into `supabase/migrations/create_cms_tables.sql`
...
4. **Verify in Supabase SQL Editor**
   ...
   -- Check that policies are created with is_admin() calls
