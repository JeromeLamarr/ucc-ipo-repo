...
function BenefitsBlockForm({ formData, updateField, addArrayItem, removeArrayItem, updateArrayItem }: any) {
  const items = Array.isArray(formData.items) ? formData.items : [];
    ...
    <div className="space-y-4">
      <div>
        ...
        <input
          ...
          value={formData.title || ''}
          onChange={(e) => updateField('title', e.target.value)}
          placeholder="e.g., Why Use UCC-IPO?"
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
        />
      </div>

      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">Section Subtitle (optional)</label>
        <textarea
...
function FaqBlockForm({ formData, updateField, addArrayItem, removeArrayItem, updateArrayItem }: any) {
  const items = Array.isArray(formData.items) ? formData.items : [];
        ...
        <div key={idx} className="bg-gray-50 p-4 rounded-lg border border-gray-200 space-y-3">
          ...
          <div>
            ...
            <input
              type="text"
              value={item.question || ''}
              onChange={(e) => updateArrayItem('items', idx, 'question', e.target.value)}
              placeholder="e.g., Who can use the platform?"
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              value={item.answer || ''}
              onChange={(e) => updateArrayItem('items', idx, 'answer', e.target.value)}
