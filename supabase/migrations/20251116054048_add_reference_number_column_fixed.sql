...
-- Generate reference numbers for existing records using a CTE
...
UPDATE ip_records
SET reference_number = 'IP-' || TO_CHAR(nr.created_at, 'YYYY') || '-' || LPAD(nr.rn::TEXT, 5, '0')
FROM numbered_records nr
WHERE ip_records.id = nr.id;
