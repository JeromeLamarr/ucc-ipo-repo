/**
 * HTML Template for Full Record Documentation (Node Server)
 * 
 * ⚠️ CRITICAL: This MUST be kept IN SYNC with frontend's src/lib/sharedHTMLTemplate.ts
 * 
 * After any changes to the template (CSS, layout, field names):
 * 1. Update src/lib/sharedHTMLTemplate.ts in frontend
 * 2. Copy exact same content here
 * 3. Test to verify PDF matches HTML download
 * 
 * This duplication is necessary because:
 * - Node server runs in separate package (server/)
 * - Can't easily import TypeScript from ../src
 * - Need self-contained service without external deps
 */
